...
static const UIRect s_rect_standby_start = {70U, 148U, 340U, 132U};
...
{
    /* Static colours – no breathing animation.
     * Normal : dark fill + cyan border/text
     * Pressed: white fill + cyan border + dark text  (instant feedback) */
    ...
    LCD_DrawThickRoundRect((uint16_t)(s_rect_standby_start.x - 6U),
                           (uint16_t)(s_rect_standby_start.y - 6U),
                           (uint16_t)(s_rect_standby_start.w + 12U),
                           (uint16_t)(s_rect_standby_start.h + 12U),
                           18U,
                           edge,
                           1U);

    Draw_Button(&s_rect_standby_start,
                fill,
                edge,
                text,
                pressed,
...
void UI_DrawStandby(bool pressed)
{
    ...
    Draw_StandbyStartButton(pressed);

    Draw_CenteredTextScaled((uint16_t)(UI_TOP_BAR_H + 34U), "TOUCH SCREEN CONTROL ONLINE", COLOR_TEXT_DIM, COLOR_BG, 2U);
}
...
{
    Draw_FaceAbortButton(pressed);
}

void UI_RedrawFaceProceedButton(bool pressed)
{
    Draw_FaceProceedButton(pressed);
}

void UI_RedrawVoiceRefreshButton(bool pressed)
{
    Draw_VoiceRefreshButton(pressed);
}
